...
class ButtonHelperTest < ActionDispatch::IntegrationTest
  ...
  test 'should be btn-prymary only compatible button when accessed in-progress or completed goals-path' do
    get goals_path(rewards: 'inprogress')
    assert_equal 'btn btn-primary', button_primary_or_not('inprogress')
    assert_equal 'btn btn-outline-primary', button_primary_or_not('completed')

    get goals_path(rewards: 'completed')
    assert_equal 'btn btn-outline-primary', button_primary_or_not('inprogress')
    assert_equal 'btn btn-primary', button_primary_or_not('completed')
  end
end
